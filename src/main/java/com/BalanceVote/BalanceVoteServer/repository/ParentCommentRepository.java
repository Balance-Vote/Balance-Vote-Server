...
public interface ParentCommentRepository extends JpaRepository<ParentComment,Long> {
    ...
    @Query(value = "SELECT parent.* FROM balance_vote_database.parent_comment parent LEFT OUTER JOIN balance_vote_database.child_comment child ON child.parent_id = parent.id", nativeQuery = true)
